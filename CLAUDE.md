...
│   │                              NEUTRAL filters — the page owns no filter state at all;
...
- `RecordsPage` takes the full scoring config (the 9 props, PlayerList-style) but **owns no filter state**: it runs `computeRankings` with neutral filters (All games/modes, full year range, no LAN, no Power Ranking) so the gear is its only input, then `computeRecords` (+ `groupEvents` for the prize card). Home-board filters deliberately never reach it.
